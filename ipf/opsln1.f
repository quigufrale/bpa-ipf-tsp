...
c
      do kt = 1, ntot
        ...
        jt = ikk(4,kt+ntota)
        found = .false.
c       Loop only while jt is in-bounds; avoid touching indx(*,jt)
c       unless we already know jt is valid
        do while (jt .gt. 0 .and. jt .le. nindxx)
c         stop scanning this block if the row tag no longer matches
          if (iabs(indx(1,jt)) .ne. kt+ntota) exit
c         check the type on column 2
c
...
c
            do i = 1, ilast
              mt = itemp(i)
              jt = ikk(5,mt+ntota)
              do while (jt .gt. 0 .and. jt .le. njndxx)
                if (iabs(jndx(1,jt)) .ne. mt+ntota) exit
c
